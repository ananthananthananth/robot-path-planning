...
def tree_search(grid, start, goal, radius, reso, heuristic_type="manhattan"):
    ...
    came_from = {}  # To store the path (came_from is used for backtracking)
    ...
    while open_set:
        ...
        # Draw the red line for each step (from previous node to current node)
            ...
            if 0 <= neighbors[0] < rows and 0 <= neighbors[1] < cols:
                if grid[neighbors[0], neighbors[1]] == 1:  # Obstacle
                    ...
                    if neighbors not in g_cost or tentative_g_cost < g_cost[neighbors]:
                        ...
                        came_from[neighbors] = current  # Store the path (where we came from)
    ...
    # Reconstruct the path efficiently using came_from
    path = []
    current_pos = goal
    while current_pos != start:
        path.append(current_pos)
        current_pos = came_from[current_pos]  # Backtrack using the came_from dictionary
...
def calc_obs(g, radius, dim):
    ...
    # Use numpy operations where possible to speed up distance calculations
    for r in range(rows):
        for c in range(cols):
            if g[r, c] == 1:
...
def main(heuristic_type="manhattan"):
    ...
    path, g_cost, overall_cost = tree_search(data, start, goal, robot_radius, resolution, heuristic_type)
...
if __name__ == '__main__':
    main(heuristic_type="manhattan")  # Can be interchanged between "manhattan" or "euclidean"
